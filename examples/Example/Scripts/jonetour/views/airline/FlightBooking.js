(function () {
    ...
    VA.views.airline.FlightBooking = function () {
        ...
        var grid_domestic_booking_info_manage_configs = null;

        var grid_domestic_booking_info_manage_paged = null;

        var initGridDomesticBookingInfoManage = function () {

            grid_domestic_booking_info_manage_configs = {
            ...
            grid_domestic_booking_info_manage_configs.data = {
                package_name: 'PK_BD_DOMESTIC_BI_MANAGE',
        ...
        var renderGridDomesticBookingInfoManage = function (conditions) {

            if (conditions) {
                grid_domestic_booking_info_manage_configs.data.p_conditions = JSON.stringify(conditions);
            }

            grid_domestic_booking_info_manage_paged = new SS.core.GridPaged();

            grid_domestic_booking_info_manage_paged.init(grid_domestic_booking_info_manage_configs);
        }


        var initControls = function () {

            initGridDomesticBookingInfoManage();

            setGridConfigFromQueryString();

            renderGridDomesticBookingInfoManage();

            //su kien click nut Xem
            ...
            jQuery(document).bind('deserializeObjectToFormView', function (e) {
                ...
                if (e.objectForm.STATUS == 1) {
                    ...
                    if (confirm('Ticketing chắc chắn nhận xử lý mã dịch vụ ' + e.objectForm.ORDER_ID + ' ?') == false) {
                        return false;
                    }
            ...
            //dem so ma dang xem hoac cho xu ly
        ...
        var setGridConfigFromQueryString = function () {
            ...
            grid_domestic_booking_info_manage_configs.data = {
                ...
                p_conditions: JSON.stringify(conditions),
                row_start: 0,
                row_end: grid_domestic_booking_info_manage_configs.page_size
            };

            grid_domestic_booking_info_manage_configs.onRenderComplete = function () {
        ...
        var getSmsBrandNameModulePartial = function (objectForm) {
            ...
            jQuery.ajax({
                ...
                success: function (data) {
                    ...
                    //console.log(objectForm.order_id_request);
                },
                error: function (http, message, exc) {
                    jwm.Alert.HideAjaxProcessing(formBoxId);
                    jwm.Alert.ShowMsg(formBoxId, -1, message + " " + exc, 'Thông báo');
        ...
        var initEnhanceFilterForm = function () {
            ...
            data_post.package_name = "PK_BD_DOMESTIC_BI_MANAGE";
            ...
            //$('#BOOKING_FROM_DATE, #BOOKING_TO_DATE').datetimepicker({
            //    format: 'd/m/Y',
            //    timepicker: false,
            //    lang: 'vn'
            ...
            //$('#BOOKING_FROM_DATE').val(from_date);//tạo giá trị ngay ban đầu

            //var to_date = $.format.date(current_date, 'dd/MM/yyyy');
            //$('#BOOKING_TO_DATE').val(to_date);
        ...
        var initRebindGridOnNewClientMessage = function () {
            jQuery(document).bind('onReceiveMessageFromHub', function (e) {
                if (e.clientMessage && e.clientMessage.Data && e.clientMessage.Data.order_id) {
                    grid_domestic_booking_info_manage_paged.renderGrid(true);
                }
            });
        }
        ...
        var getUserInfoRoundAction = function (callback) {
            ...
            jwm.Alert.ShowAjaxProcessing(_configs.form_id);
            jQuery.ajax({
                url: _configs.service_wss_vietair_tv_url + "/MembershipExt.asmx/GetTwmb001RoundAction",
                dataType: 'json',
                type: 'POST',
                data: {
...
//---------------------------------------------------
//OTHER SCRIPTS
var getInlineCellStatus = function (data) {
    var value = "";
    if (data.STATUS == 2) {
...
var getInlineCellSourceBookingIdStatus = function (data) {
    var html = '';
    if (data.SOURCE_BOOKING_ID == 1 || data.SOURCE_BOOKING_ID == 3) {
        ...
        if (data.BOOKING_CODE && data.BOOKING_CODE.indexOf('TAV') >= 0) {
            //vnisc
            html = '';
        }
        else {
            //source at
            html = '<img src="/Content/images/icon_booking/a-t.png" alt="AT" />';
        }
...
var getShortDateTimeString = function (longDate) {
    if (isEmpty(longDate)) {
        return '';
    }
