(function () {
    ...
    VA.views.chartDomestic.ChartBookingBetweenMembers = function () {
        ...
        var initTitleChart = function () {
            ...
            $('.title-charts').text('Thống kê mã dịch vụ nội địa giữa các nhân viên');
        ...
        var initControlChartBookingBetweenMembers = function () {
            ...
            $('#btn_view_chart_all_flight_booking').bind('click', function () {
                ...
                var mb_id = $('#all_members').val().join(',');

                var from_date = $('#date_start').val();
                var to_date = $('#date_end').val();

                getDataChartBookingBetweenMembers(mb_id, from_date, to_date);

            });
        }
       
        var getDataChartBookingBetweenMembers = function (mb_id, from_date, to_date) {
            getDataBookingBetweenMembers(mb_id, from_date, to_date, prepareDataBookingBetweenMembers)
        }

        var getDataBookingBetweenMembers = function (mb_id, from_date, to_date, callback) {

            jQuery.ajax({
                url: _configs.service_wss_vietair_tv_url + "/listinfo.asmx/GetDataFlyBookingInfoByMutilMembers",
                dataType: "jsonp",
                data: {
        ...
        var prepareDataBookingBetweenMembers = function () {
            var data = {
            };
                ...
                tooltip: {
                    formatter: function () {
                        return '<b>' + this.x + '</b><br/>' +
                            this.series.name + ': ' + Math.round(this.y * 100 / this.point.stackTotal) + '% (' + this.y + ' MDV)<br/>' +
                            'Tổng: ' + this.point.stackTotal + ' MDV';
                    }
                },
                plotOptions: {
                    column: {
                        ...
                        dataLabels: {
                            ...
                            // },
                            formatter: function () {
                                //return '<span style="color: white">' + this.series.name + ': ' + Math.round(this.y * 100 / this.point.stackTotal) + '% (' + this.y + ' MDV)</span><br/>'
                                return '<span style="color: white">' + Math.round(this.y * 100 / this.point.stackTotal) + '% (' + this.y + ' MDV)</span><br/>'
                            }
                        }
                    }
                },
