(function () {
    ...
    VA.views.accounting.ManageTicketingBill = function () {
            ...
            jQuery.ajax({
                ...
                success: function (data) {
                    ...
                    if (data != null && data.TypeMsg > 0) {
                        ...
                        $('#tmp_list_bank_id').tmpl(data.Data.CURSOR_DATA).appendTo("#BANK_ID");
            ...
            $('#CUSTOMER_CODE').autoComplete({
                ...
                source: function (term, suggest) {
                    ...
                    $.ajax({
                        ...
                        data: {
                            package_name: 'PK_BD_ACC_TICKETING_BILL',
                            object_name: 'GET_LIST_CUSTOMER_CODES',
                            P_CUSTOMER_CODE: term
                        },
                        success: function (resp) {
                            var choices = [];
                            $.each(resp.Data.CURSOR_DATA, function (i, item) {

                                var item_array = [];

                                item_array.push(item.CUSTOMER_CODE);
                                item_array.push(item.COMPANY_NAME_EXP);
                                item_array.push(item.TAX_CODE);

                                choices.push(item_array);
                            });
                            suggest(choices)
                        }
                    });
                    ...
                    var html = '';
                    html = '<div class="autocomplete-suggestion" data-customer-code="' + item[0] + '"><div class="suggestion-label1">' + item[0] + '</div> <div class="suggestion-label2">' + item[1] + ' - MST: ' + item[2] + '</div></div>';
                    return html;

                }, onSelect: function (e, term, item) {

                    $('#CUSTOMER_CODE').val(item.data('customer-code'));
                    ...
                    $.ajax({
                        ...
                        data: {
                            package_name: 'PK_BD_ACC_TICKETING_BILL',
                            object_name: 'GET_LIST_CUSTOMER_BILL_INFO',
                            P_CUSTOMER_CODE: item.data('customer-code')
                        },
                        success: function (resp) {

                            var customer_info = resp.Data.CURSOR_DATA[0];
                            $('#COMPANY_NAME_EXP').val(customer_info.COMPANY_NAME_EXP);
                            $('#TAX_CODE').val(customer_info.TAX_CODE);
                            $('#ADDRESS_EXP').val(customer_info.ADDRESS_EXP);
                            $('#PEOPLE_RECEIVE').val(customer_info.PEOPLE_RECEIVE);
                            $('#MOBILE_EMAIL').val(customer_info.MOBILE_EMAIL);
                            $('#COMPANY_IMP').val(customer_info.COMPANY_IMP);
                            $('#ADDRESS_IMP').val(customer_info.ADDRESS_IMP);
        ...
        var initMoneyReality = function () {

            $('#MONEY_REALITY').blur(function () {
                money_reality = $('#MONEY_REALITY').val();
                var money_request = $('#MONEY_REQUESTED_INVOICE').val();
                if (money_request.length == 0) {
                    $('#MONEY_REQUESTED_INVOICE').val(money_reality);
                }
        ...
        var initSeDateTicketingBill = function () {
        ...
        var addTickeTingBill =  function () {
            ...
            data_post.object_name = 'INSERT_TICKETING_BILL';
        ...
        var updateTickeTingBill = function () {
            ...
            data_post.object_name = 'UPDATE_TICKETING_BILL';
        ...
        var initControls = function () {
            ...
            jQuery(form_info.button_add_update_id).click(function () {
                ...
                if (jQuery(form_info.input_key_id).val() >= "0") {
                    ...
                    return updateTickeTingBill();
                }
                else {

                    return addTickeTingBill();
